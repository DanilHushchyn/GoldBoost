...
class Command(BaseCommand):
    ...
    def _create_games(self):
        ...
        for i in range(3):
            game = Game.objects.create(
                ...
                logo_filter=File(filter_logo,filter_logo.name),
                logo_product=File(product_logo),
                logo_product_alt_en=self.fake_en.word(),
                logo_product_alt_ua=self.fake_ua.word(),
                logo_filter_alt_en=self.fake_en.word(),
                logo_filter_alt_ua=self.fake_ua.word(),
                order=i,
            )
            game.save()
        ...
        for game in Game.objects.all():
            for j in range(5):
                page = CatalogPage.objects.create(
                    ...
                    title_ua=self.fake_ua.word().capitalize(),
                    description_en=self.fake_en.text(max_nb_chars=500).capitalize(),
                    description_ua=self.fake_ua.text(max_nb_chars=500).capitalize(),
                    parent=None,
                    game_id=game.id,
                    order=j,
                )
                for i in range(4):
                    CatalogTabs.objects.create(
                        ...
                        content_ua=self.fake_ua.text(max_nb_chars=500).capitalize(),
                        order=i,
                        catalog_id=page.id
                    )
    ...
    def _create_main_page(self):
        obj = {
            ...
            "subscribe_form_text_ua": "Підпишіться на нашу електронну розсилку та отримайте ЗНИЖКУ 10% на перше "
                                      "замовлення!",
        ...
        for j in range(20):
            Review.objects.create(
                ...
                comment_ua=self.fake_ua.text(max_nb_chars=120).capitalize(),
            )
        for game in Game.objects.all():
            ...
            for i in range(6):
                News.objects.create(
                    ...
                    description_ua=self.fake_ua.text(max_nb_chars=150).capitalize(),
                    image_alt_en=self.fake_en.word(),
                    image_alt_ua=self.fake_ua.word(),
                    game_id=game.id,
                    image=File(image),
                )
        for j in range(3):
            ...
            WhyChooseUs.objects.create(
                ...
                icon_alt_ua=self.fake_ua.word().capitalize(),
            )
        ...
        for page in CatalogPage.objects.all():
            for i in range(3):
                product = Product.objects.create(
                    ...
                    subtitle_ua=self.fake_ua.sentence(nb_words=3).capitalize(),
                    description_en=self.fake_en.text(max_nb_chars=150).capitalize(),
                    description_ua=self.fake_ua.text(max_nb_chars=150).capitalize(),
                    ...
                    image=File(image),
                    card_img=File(card_image),
                    card_img_alt_en=self.fake_en.word(),
                    card_img_alt_ua=self.fake_ua.word(),
                    image_alt_en=self.fake_en.word(),
                    image_alt_ua=self.fake_ua.word(),
                )
                product.save()
            ...
            # for product in Product.objects.filter(price_type='range'):
            #     for i in range(4):
